...
List<Exercise> _loadBundledExercises() {
  final jsonStr = File('assets/data/exercise_library.json').readAsStringSync();
...
void main() {
  ...
  group('PlanEngine.buildWeeklySchedule', () {
    test('fullBody with freq 1 has 1 workout day', () {
      final schedule = PlanEngine.buildWeeklySchedule(
      ...
      for (final split in TrainingSplit.values) {
        for (var freq = 1; freq <= 6; freq++) {
          ...
          expect(
            schedule.length,
            7,
            reason: '$split freq=$freq should be 7 days',
          );
        }
      }
    });
  ...
  group('PlanEngine.trainingParameters', () {
    test('buildMuscle beginner → 3 sets, 10 reps', () {
      final p = PlanEngine.trainingParameters(
    ...
    test('exercisesPerSession scales with level', () {
      final beg = PlanEngine.trainingParameters(
        FitnessGoal.maintain,
        ExperienceLevel.beginner,
      );
      final adv = PlanEngine.trainingParameters(
        FitnessGoal.maintain,
        ExperienceLevel.advanced,
      );
  ...
  group('PlanEngine.selectExercises', () {
    ...
    test('filters by available equipment', () {
      ...
      for (final ex in selected) {
        expect(
          ex.equipment != Equipment.barbell,
          true,
    ...
    test('returns non-empty list for all day types', () {
      for (final dt in WorkoutDayType.values) {
        ...
        expect(
          warmup.isNotEmpty,
          true,
          reason: '$dt warmup should not be empty',
        );
      }
    });
  });
    ...
    test('generates 7-day plan with correct structure', () {
      ...
      for (final day in workDays) {
        expect(
          ...
          true,
          reason: '${day.dayType} should have exercises',
        );
      }
    });

    test('bundled library does not generate empty workout days', () {
      final profile = UserProfile(
        weeklyFrequency: 3,
        goal: FitnessGoal.buildMuscle,
      ...
      final plan = PlanEngine.generatePlan(profile, _loadBundledExercises());
      final emptyWorkDays = plan.days.where(
        (d) => d.dayType != WorkoutDayType.rest && d.exercises.isEmpty,
      );

      expect(emptyWorkDays, isEmpty);
    });
  });
}
