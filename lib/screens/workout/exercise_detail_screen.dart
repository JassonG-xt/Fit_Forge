...
class ExerciseDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (exercise == null) {
      return Scaffold(
        appBar: AppBar(),
        body: Center(child: Text('动作未找到', style: theme.textTheme.bodyMedium)),
      );
    }
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ChipTag(
                  label: exercise.bodyPart.displayName,
                  selected: true,
            ...
            ...exercise.formCues.map(
              (c) => _tipRow(theme, c, Icons.check_circle, AppColors.accent),
            ),
            const SizedBox(height: AppSpacing.md),

            // 避免借力
            _section(theme, '避免借力', Icons.warning_amber),
            ...exercise.antiCheatTips.map(
              (t) => _tipRow(theme, t, Icons.warning_amber, AppColors.warning),
            ),
            const SizedBox(height: AppSpacing.md),
            ...
            ...exercise.commonMistakes.map(
              (m) => _tipRow(theme, m, Icons.cancel, AppColors.danger),
            ),
            const SizedBox(height: AppSpacing.md),
              ...
              children: [
                Expanded(
                  child: SectionCard(
                    child: StatNumber(
                      value:
                          '${exercise.recommendedSetsMin}-${exercise.recommendedSetsMax}',
                      label: '组数',
                      fontSize: 20,
                    ),
                  ),
                ...
                Expanded(
                  child: SectionCard(
                    child: StatNumber(
                      value:
                          '${exercise.recommendedRepsMin}-${exercise.recommendedRepsMax}',
                      label: '次数',
                      fontSize: 20,
                ...
                return ListTile(
                  ...
                  onTap: () => Navigator.push(
                    ...
                    MaterialPageRoute<void>(
                      builder: (_) => ExerciseDetailScreen(exerciseId: alt.id),
                    ),
                  ),
                  dense: true,
                );
  ...
  static IconData _bodyPartIcon(BodyPart part) => switch (part) {
    ...
    BodyPart.biceps ||
    BodyPart.triceps ||
    BodyPart.forearms => Icons.front_hand,
