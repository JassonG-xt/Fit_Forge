...
export 'app_state_import_preview.dart';
...
class AppState extends ChangeNotifier {
  ...
  static const int maxImportJsonChars =
      AppStateImportExportService.maxImportJsonChars;
  ...
  List<double> weekActivityForCurrentWeek({DateTime? now}) =>
      SessionQueries.weekActivity(completedSessions, now: now ?? _clock.now());
  ...
  WorkoutDay? get todayWorkout {
    ...
    final weekday = _clock.now().weekday; // 1=Monday
    try {
  ...
  AppStateSnapshot _snapshot() {
    return AppStateSnapshot(
      profile: _profile,
      activePlan: _activePlan,
      sessions: List<WorkoutSession>.of(_sessions),
      bodyMetrics: List<BodyMetric>.of(_bodyMetrics),
      achievements: List<Achievement>.of(_achievements),
      hasCompletedOnboarding: _hasCompletedOnboarding,
      themeMode: _themeMode,
    );
  }
