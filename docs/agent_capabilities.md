...
- **eval suite 不是分数表**：`coach_agent_eval_cases.json` 是行为契约，49 active / 4 expectedGap（53 cases）；保留 expectedGap 作为 regression signal，不为「全绿」放宽守护。
...
- **recovery-aware mutation routing 很窄**：只有明确要求“压缩 / 缩短到 X 分钟”等带具体分钟数的恢复相关请求才会路由到现有 `compressWorkout`；“有点累，要不要休息”或“改轻一点”这类模糊恢复问题保持 non-mutating。所有 mutation 仍需要确认、trusted `sourceContextHash` 和 `LocalAgentActionExecutor`。
