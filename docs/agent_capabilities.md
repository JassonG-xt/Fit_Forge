...
- **本地 Markdown 周报导出是产品 polish，不是 Agent 能力**：`lib/reports/weekly_report_builder.dart` 是确定性纯函数，从 `AppState`（profile / activePlan / 本周完成的 session / 计算出的 nutrition target）派生 Markdown，Settings → 数据管理 → “复制本周报告” 入口复制到剪贴板。周报可包含本地已有的结构化 `weeklyReview` 字段（summary / observations / nextWeekSuggestions / riskNotes），但导出本身仍是确定性、non-mutating 的本地流程。不调用 LLM、不调用 backend、不上传、不写入 `AppState`、不进入 `LocalAgentActionExecutor` 路径；任何场景下都包含 Safety Note；不是医疗建议；不替代结构化 `weeklyReview` action。
