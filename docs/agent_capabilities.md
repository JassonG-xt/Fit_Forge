...
| Action | Stage | Status |
|---|---|---|
| `moveWorkoutSession` | Stage 3-3 — Flutter mock routing | **Flutter mock 路由 + 本地 executor 已就位，但 backend / real-provider 路由与 eval coverage 仍未接入**：`AgentActionType.moveWorkoutSession` 已加入枚举；`parseMoveWorkoutSessionPayload` 做严格 payload 校验（`fromDayOfWeek` / `toDayOfWeek` 必填 1-7、不能相同；可选 `reason` 字符串）；`AgentActionPreviewer.previewMoveWorkoutSession` 返回 weekday-level `MovePreview`；`LocalAgentActionExecutor` 在用户确认 + trusted `sourceContextHash` 命中的前提下，把源日训练完整移到目标日（保留 sets/reps/rest 与确定性 1..7 排序），源日转为 rest；**目标日已有训练时拒绝执行**（不自动合并、不交换、不追加）；源日无训练、缺失 / stale `sourceContextHash`、未带用户确认的请求都不会修改 `AppState`。Stage 3-3 在 `MockAgentClient` 加了**仅** explicit weekday-to-weekday 的确定性路由（如 `把周一训练挪到周三`），命中后发出 `requiresConfirmation=true` + trusted `sourceContextHash` + `fromDayOfWeek` / `toDayOfWeek` payload；可选 `reason` 仅在前缀含明显恢复关键字（如 `累` / `太密` / `恢复`）时回填。模糊请求（`帮我调整一下训练` / `把训练挪一下`）保持 non-mutating，高风险症状仍优先 `safetyResponse`，"今天→明天" 因 mock 缺乏确定性当前日期源而留作后续设计提案。**Backend、real provider 与 eval 套件仍未路由此 action**——HTTP / real 链路中正常 Agent 对话流程不可达；backend prompt routing、real-provider routing、eval coverage 留给后续 PR。详见 `docs/move_workout_session_design.md`。 |
