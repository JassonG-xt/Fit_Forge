...
> Stage 3-2 local executor support (historical): `LocalAgentActionExecutor` added deterministic `moveWorkoutSession` execution for explicit weekday-to-weekday movement. Target-day conflicts reject without auto-merge / swap / append, and missing-source-day, missing/stale hash, and unconfirmed-action paths reject without mutating `AppState`. Later Stage 3 work made the action reachable through mock/backend/provider paths with eval and smoke coverage.

> Stage 3-3 Flutter mock routing (historical): `MockAgentClient` added explicit weekday-to-weekday `moveWorkoutSession` routing for local mock mode. Later Stage 3 work closed backend deterministic routing, eval contract coverage, real-provider prompt support, and a sanitized manual smoke scorecard; see `docs/move_workout_session_phase_summary.md` for the current consolidated status.

> Stage 3-4 backend deterministic routing (historical): `agent_backend/agents/coach_agent.py` mirrored the Flutter mock matcher for explicit weekday-to-weekday `moveWorkoutSession`, added the action to mutation safety handling, and added strict backend payload validation. Real-provider prompt support and eval/smoke coverage landed in later Stage 3 PRs.

> Stage 3-5 eval contract coverage (historical): `agent_backend/evals/coach_agent_eval_cases.json` added 5 active cases pinning the `moveWorkoutSession` boundary: explicit weekday-to-weekday mutation, recovery-prefix weekday move, vague move no-mutation, today→tomorrow no-mutation, and safety-over-move. Later phases expanded the overall eval baseline to 77 total / 73 active / 4 expectedGap.
